...
class PlanilhaVendas(BaseModel):
    ...
    AdSet_name: str = Field(None, description="Nome do conjunto de anúncios")
    Amount_spent: float = Field(0.0, ge=0, le=1200.00, description="Valor gasto no anúncio")
    Link_clicks: float = Field(None, description="Número de cliques no link", nullable=True)
    Impressions: float = Field(0, description="Número de impressões do anúncio", nullable=True)
    Conversions: float = Field(None, description="Número de conversões registradas", nullable=True)
